...
interface Ingredient { name: string; state: IngState; full: string; per: string }
...
const MEALS: Record<MealTab, Meal[]> = {
  breakfast: [
    {
      ...
      reheat: [{ method: "cold", desc: "No reheat needed — make fresh each morning in ~10 min. Oats cook in 3–4 min on stovetop or microwave." }],
    },
    {
      name: "Greek Yogurt Power Bowl",
      subtitle: "Fage 0% · granola · banana · honey · peanut butter · protein shake",
      icon: "🥣", tag: "Zero cooking · 3 min assembly",
      ...
      note: "Literally 3 minutes. Fairlife Core Power has 26g P per bottle and tastes good cold. PB2 adds peanut butter flavor at 80% less fat than regular peanut butter.",
      reheat: [{ method: "cold", desc: "No reheat — assembled straight from the fridge. This is a feature, not a bug." }],
    },
    {
      name: "High-Protein Breakfast Burrito",
      subtitle: "Carb Balance tortillas · eggs · black beans · salsa",
      icon: "🌯", tag: "Best meal-prep option",
    ...
    },
    {
      ...
      icon: "🧇", tag: "Feels like a cheat meal",
      ...
      note: "Kodiak Cakes: 14g P per 1/2 cup dry — by far the highest-protein waffle mix available. Batter keeps refrigerated 2 days.",
      reheat: [
  ...
  lunch: [
    {
      ...
      icon: "🥣", tag: "4 servings · Office-safe",
    ...
    },
    {
      ...
      icon: "🥡", tag: "4 servings · Works cold",
    ...
    },
    {
      ...
      icon: "🫙", tag: "4 servings · Top pick · Office-safe",
    ...
    },
    {
      ...
      note: "⚠️ Do NOT bring to the office. Prep Wednesday evening — good Thu and Fri only (3 days max refrigerated).",
      reheat: [
        ...
        { method: "cold", desc: "Cold salmon over the sweet potato and broccoli is actually solid." },
      ],
    },
    {
      name: "Cajun Shrimp & Quinoa",
      subtitle: "Frozen shrimp · quinoa · bell peppers · Cajun seasoning",
      icon: "🍤", tag: "3 servings · Home only · Leanest",
      ...
      note: "⚠️ Home lunches only. Add 2 rice cakes on the side to hit carb targets on high-demand days.",
      reheat: [
        { method: "stovetop", desc: "Best option — high heat, 2 min in a pan. Shrimp reheats fast; pull it off the second it's hot or it turns rubbery." },
        { method: "cold", desc: "Quinoa holds up cold and shrimp is fine room temp." },
      ],
    },
  ],
  dinner: [
    {
      ...
      reheat: [
        { method: "stovetop", desc: "Slice chicken first, then heat in a pan over medium with a tbsp of water or chicken broth, 2–3 min covered. Keeps it moist." },
    ...
    },
    {
      ...
      note: "96/4 beef = ~145 kcal per 100g, nearly as lean as chicken breast but significantly more iron, zinc, and B12.",
      reheat: [
        { method: "stovetop", desc: "Back in the pan over medium, splash of water to loosen, stir 3–4 min." },
        { method: "oven", desc: "Oven-safe dish, covered at 350°F for 12 min." },
      ],
    },
    {
      name: "Garlic Ginger Shrimp & Bok Choy Stir-Fry",
      subtitle: "Shrimp · edamame · bok choy · soy-ginger sauce",
      icon: "🦐", tag: "Lightest option · Best for Wed",
      ...
      reheat: [
        { method: "stovetop", desc: "High heat, 2 min max — shrimp goes rubbery fast. Add a splash of soy sauce and toss constantly." },
        { method: "cold", desc: "Edamame and bok choy are both fine cold or room temp." },
      ],
    },
    {
      name: "Pork Tenderloin with Brussels & White Beans",
      subtitle: "Pork tenderloin · Brussels sprouts · cannellini beans",
      icon: "🥦", tag: "Different protein source",
      ...
      note: "Pork tenderloin = ~143 kcal per 100g, 26g P — nearly identical macros to chicken breast. Completely different texture and flavor.",
      reheat: [
        { method: "oven", desc: "Best option — 325°F for 12–15 min covered with foil. Pork dries out fast; low and slow is the move." },
        { method: "stovetop", desc: "Slice pork thin, medium heat with a tbsp of water, 2–3 min covered." },
        { method: "airfryer", desc: "400°F for 4–5 min. The Brussels come back to life and the pork stays juicy." },
      ],
    },
    {
      name: "Turkey & Black Bean Stuffed Peppers",
      subtitle: "Ground turkey · bell peppers · black beans · salsa",
      icon: "🫑", tag: "Batch-friendly",
      ...
      note: "Make the filling in bulk, stuff peppers as needed through the week.",
      reheat: [
        { method: "oven", desc: "Best option — 375°F for 18–20 min covered with foil, then uncover for 5 min." },
        { method: "stovetop", desc: "Add ½ cup water to a lidded pan, place peppers in, cover and steam over medium for 10–12 min." },
...
const TAB_META: Record<MealTab, { label: string; color: string; bg: string; target: string }> = {
...
function MealCard({ meal, tab, defaultOpen }: { meal: Meal; tab: MealTab; defaultOpen?: boolean }) {
...
type FuelTab = 'targets' | 'meals' | 'supplements'
...
export default function FuelPage() {
  ...
  const mealMeta = TAB_META[mealTab]
  ...
  return (
    <div className="hub-page">
      ...
      <div className="mh-tab-bar">
        {(['targets', 'meals', 'supplements'] as FuelTab[]).map(t => (
          <button
            key={t}
            className={`mh-tab-btn${tab === t ? ' active' : ''}`}
            onClick={() => setTab(t)}
          >
            {t === 'targets' ? 'Targets' : t === 'meals' ? 'Meals' : 'Supplements'}
          </button>
        ))}
      </div>
      ...
      {tab === 'targets' && (
        <>
          ...
          <div style={{ marginBottom: 24 }}>
            <div className="section-hdr"><span className="ptitle">Cronometer Data Upload</span></div>
            <div
              onDrop={onDrop}
      ...
      {tab === 'meals' && (
        <>
          ...
          {/* Meal sub-tab bar */}
          <div className="mh-tab-bar" style={{ marginBottom: 0 }}>
            {(Object.keys(TAB_META) as MealTab[]).map(t => (
              <button
                key={t}
                className={`mh-tab-btn${mealTab === t ? ' active' : ''}`}
                onClick={() => setMealTab(t)}
                style={mealTab === t ? { color: TAB_META[t].color } : {}}
              >
          ...
          <div className="mh-panel-header">
            <h3 style={{ color: mealMeta.color }}>{mealMeta.label}</h3>
            <span className="mh-target-badge">{mealMeta.target}</span>
          </div>

          <div className="mh-meals-grid">
            {MEALS[mealTab].map((meal, i) => (
              <MealCard key={`${mealTab}-${i}`} meal={meal} tab={mealTab} defaultOpen={i === 0} />
            ))}
          </div>
        </>
      )}
